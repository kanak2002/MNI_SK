...
def is_third_party_trusted(third_party: str, persona_name: str):
    ...
    normalized = third_party.strip().title()

    if normalized not in trust_db:
        # convenience_first trusts unknown parties
        if persona_name == "convenience_first":
            return True, "Trusted by default under Convenience persona"
        return False, "Unknown third party"

    trusted_by = trust_db[normalized]["trusted_by"]
